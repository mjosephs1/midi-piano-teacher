...
def load_chord_settings(group_names: list[str]) -> dict:
    ...
    defaults = {"group_enabled": {name: True for name in group_names}, "sharps_mode": "include"}
    ...
    try:
        ...
        return {"group_enabled": group_enabled, "sharps_mode": sharps_mode}
    except Exception:
        return defaults


def save_chord_settings(group_enabled: dict, sharps_mode: str) -> None:
    with open(SETTINGS_FILE, "w") as f:
        json.dump({"group_enabled": group_enabled, "sharps_mode": sharps_mode}, f, indent=2)
